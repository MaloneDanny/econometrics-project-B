...
cleanprecipdata = cleanprecipdata|>
  filter(year != 2023)
#this begins to do calculations on the average rainfall of a county
cleanprecipdata = cleanprecipdata|>
  group_by(fips)|>
  mutate(meanprecip = mean(ave))
#calculate the difference between the average amount of rainfall in a year and the average over all years, and adds it to the data frame
diffprecip = cleanprecipdata$ave - cleanprecipdata$meanprecip
cleanprecipdata = cbind(cleanprecipdata,diffprecip)
cleanprecipdata = cleanprecipdata|>
  rename(diff = ...18)
#calculates the percent difference between the year average and overall average, standard deviation of the averages by county, and then determines that a drought is occuring in a county over a year by defining a drought as a difference between year average and overall average which is greater than one standard deviation.
cleanprecipdata = cleanprecipdata |>
  mutate(percent = 100*(diff/meanprecip))|>
  mutate(sd = sd(ave))|>
  mutate(drought = as.integer(diff < -sd))
#this saves the finished data
write.csv(cleanprecipdata, file = "precipdata")
```
