  ...
  cleandat = cleandat|>
    ...
    mutate(droughtstart = ifelse(drought == 1 & lag(drought) == 0, 1, 0),
           droughttime = ifelse(droughtstart == 1, 0,
                                ifelse(lag(droughtstart, n = 1) == 1, 1,
                                ifelse(lag(droughtstart, n = 2) == 1, 2,
                                ifelse(lag(droughtstart, n = 3) == 1, 3,
                                ifelse(lag(droughtstart, n = 4) == 1, 4,
                                ifelse(lag(droughtstart, n = 5) == 1, 5,
                                ifelse(lead(droughtstart, n = 1) == 1, -1,
                                ifelse(lead(droughtstart, n = 2) == 1, -2,
                                ifelse(lead(droughtstart, n = 3) == 1, -3,
                                ifelse(lead(droughtstart, n = 4) == 1, -4,
                                ifelse(lead(droughtstart, n = 5) == 1, -5, 99))))))))))))
cleandat = cleandat|>
  group_by(fips)|>
  mutate(droughttime = ifelse(droughtstart == 1, 
                              lag(droughttime, n = 1) == 0, 99))

#creates a continuous event time variable
  cleandat = cleandat|>
    group_by(fips)|>
    mutate(droughttime = ifelse(drought == 1, lag(drought) == 0, -predrought,
                                ifelse(drought == 1, lead(drought))
?ifelse
